...
def confirm_modification_SALES(Id_Sales, Id_Volume, new_data_entry, table_name, modify_data_window):
    ...
    try:
        cursor.execute(f"UPDATE {table_name} SET Quantity = ? WHERE Id_Sales = ? AND Id_Volume = ?", (new_data_value, id_sales_value, id_volume_value))
        conn.commit()
        #Verificar que el .execute() fue un exito
        if cursor.rowcount == 0:
...
def invert_data(table_name, window):
    ...
    def confirm_invertion():
        ...
        cursor.execute(f"SELECT {table_name}.{column_name_value} FROM {table_name} WHERE Id_{table_name} = ?", id_value )
        ...
        try:
            cursor.execute(f"UPDATE {table_name} SET {column_name_value}  = ? WHERE Id_{table_name} = ?", (inverted_data, id_value))
            conn.commit()
            if cursor.rowcount > 0:
